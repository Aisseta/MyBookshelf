...
const UserLibrary = require('./models/UserBook');
...
app.post('/add-booksusers/', async (req, res) => {
  console.log('Requête ajout de livres reçue.');

  try {
    const newBooks = req.body;
    const userLibrary = await UserLibrary.findOneAndUpdate({}, { $push: { books: { $each: newBooks } } }, { upsert: true, new: true }).select('books.title books.author'); ;
    res.json(userLibrary);
  } catch (error) {
    console.error('Erreur lors de l\'ajout des livres:', error);
    res.status(500).send('Erreur lors de l\'ajout des livres');
  }
});
